...
The build creates `dist\SeniorConnect-Windows`. Copy that whole folder to the Windows PC without moving files out of it, then double-click `Start-SeniorConnect.bat`; it runs `SeniorConnectServer.exe` in a visible server window, opens the browser, writes startup failures to `seniorconnect-error.log`, and stores SQLite data/uploads next to the executable for easy backup.
